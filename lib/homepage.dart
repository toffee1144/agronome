...
class AgronomeHomePage extends StatefulWidget {
  final ValueNotifier<int> tabNotifier;
  const AgronomeHomePage({super.key, required this.tabNotifier});
  ...
  @override
  State<AgronomeHomePage> createState() => _AgronomeHomePageState();
}

class _AgronomeHomePageState extends State<AgronomeHomePage> {
  final store = MissionStore.instance;

  SurveyRecommendation? _reco;
  bool _loading = false;
  Timer? _timer;
  ...
  Future<void> _fetchReco() async {
    if (_loading) return;
    _loading = true;

    try {
      final data = await SurveyApi.fetchSecondLatestOrLatest();
      if (mounted) {
        setState(() => _reco = data);
      }
    } catch (_) {
  ...
  BoxDecoration _cardDeco({double r = 24}) {
  ...
  Widget build(BuildContext context) {
    return MediaQuery(
      ...
      child: Scaffold(
        backgroundColor: AgronomeHomePage.bg,
        body: SafeArea(
                ...
                const Center(
                  child: Text(
                    ...
                    style: TextStyle(
                      ...
                      color: AgronomeHomePage.textPrimary,
                    ),
                  ),
                ),
                          ...
                          children: [
                            ...
                            Text(
                              ...
                              style: TextStyle(
                                fontSize: 14,
                                height: 1.45,
                                color: AgronomeHomePage.textSecondary,
                              ),
                            ),
                            SizedBox(height: 14),
                            Text(
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      const Row(
                        children: [
                          Icon(Icons.spa_outlined, size: 22, color: AgronomeHomePage.textPrimary),
                          SizedBox(width: 10),
                          Expanded(
                            child: Text(
                      ...
                      Row(
                        ...
                        children: [
                          if (_reco != null)
                            Text(
                              SurveyApi.priorityText(_reco!.priorityLevel),
                              style: const TextStyle(
                                fontSize: 12,
                                fontWeight: FontWeight.w700,
                                color: AgronomeHomePage.green,
                    ...
                    builder: (context, _) {
                      ...
                      return Column(
                        ...
                        children: [
                          const Row(
                            children: [
                              Icon(Icons.assignment_outlined, size: 22, color: AgronomeHomePage.textPrimary),
                              SizedBox(width: 10),
                              Expanded(
                                child: Text(
                          ...
                          const SizedBox(height: 14),
                          if (next != null) _NextMissionCard(next),
                          ...
                          InkWell(
                            onTap: () {
                              Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => const AllMissionsPage()),
                              );
                            },
                            child: const Text(
                              'View All Missions',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            'Scheduled mission for ${m.missionType}',
            style: const TextStyle(
              fontSize: 14,
              height: 1.45,
              color: AgronomeHomePage.textSecondary,
